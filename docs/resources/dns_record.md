...
- `mx_pref` - (Optional) MX preference, lower being preferred, between `0` and `255`. Defaults to `10`. Changed in place.
...
`10` whatever is sent, so a value set there is ignored rather than fought over:
sending it would make the post-write check compare it against the API's `10` and
report the write as a lost race.
