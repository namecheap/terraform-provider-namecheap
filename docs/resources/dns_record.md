...
2. After writing, the zone is re-read and compared. A change lost to a writer
   *outside* the run — another Terraform run, a script, the dashboard — surfaces
   as an error and is retried, rather than silently vanishing.

That second point is worth internalising: a concurrent writer can still cost you
an apply, it just cannot cost you a record silently.
...
- `mx_pref` - (Optional) MX preference, lower being preferred. Applies to `MX` records only. Defaults to `10`, which is what the API reports for every other type.

`domain`, `hostname` and `type` are the record's identity, so changing any of
them replaces the record rather than editing it.
